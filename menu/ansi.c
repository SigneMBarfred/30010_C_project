...
// WORK IN PROGRESS a function that moves the cursor to the user specified location (similar to gotoxy but get pos from terminal input)
int setCursorPos(){
	int x;
	int y;
	printf("Enter x and y cursor position values (integers):");
	scanf(" %d %d", &x, &y); //& before x and y points to memory
...
void window(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, const char *style, const char *title){
	...
	for (int i = 0; i < x2 - x1 - 4; i++){  //now for the line where we loop over the distance in increments. notice we subtract 2 from the length bc those spaces are filled by the corners
		if (i== titlePos){
			printf("|%s|",title);
			i += strlen(title) - 1;
		} else{
