...
uint8_t readJoy(){
	...
	joy |= readSinglePin(GREY_KEY) << 4;
	joy |= readSinglePin(RED_KEY) << 5;
...
uint16_t potf(){
    ...
    // Warning! Hardware error occured while testing: y-axis not guranteed to work
	// Disabled in main.c until hardware issue resolved.
	if(adcValuePA0 >= 870){
		r = 1;
		}
		else if(adcValuePA0 <= 835){
		r = 2;
		}
